  ...
  login(username, password) {
    ...
    if (!this.securityManager.checkDeviceAuthorization()) {
      return {
        success: false,
        message:
          "This device is not authorized. Authorized devices: Device-001, Device-002, Device-003",
      };
    }
...
class TestUIManager {
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.retakeTest());
    document
      .getElementById("back-to-levels-btn")
      .addEventListener("click", () => this.backToLevels());
  ...
  startLevel(level) {
    if (
      ...
      questions[`level${level}`].length === 0
    ) {
    ...
    this.testManager.onUpdateProgress = (progress) =>
  ...
  loadQuestion(index) {
    ...
    document.getElementById("question-number").textContent =
      `Question ${index + 1}/${this.testManager.questions.length}`;
  ...
  updateCheckButton() {
    ...
    if (status.answered) {
      ...
      checkBtn.style.background = status.correct
        ? "var(--correct-color)"
        : "var(--incorrect-color)";
    } else {
  ...
  showResults() {
    ...
    if (percent >= 80) {
      ...
      certStatus.textContent =
        "📚 Keep practicing. You need 80% to be certification ready.";
  ...
  loadUserProgress() {
    ...
    if (saved) {
      try {
        ...
        document.getElementById("level1-stats").textContent =
          `${data.correct}/49`;
      } catch (e) {
  ...
  saveProgress() {
    ...
    localStorage.setItem(
      `ic3_level${this.currentLevel}_progress`,
